...
def gns3_get_appliances_names_and_id(gns3_server):
    ...
    if show_appliances:
        ...
        for appliance in show_appliances_dict:
            ...
            gns3_appliances_dict.update(
                {gns3_appliance_name: gns3_appliance_id})
...
def gns3_show_projects(gns3_server):
    ...
    if show_projects:
        ...
        projects_count = 0
        t_projects = PrettyTable(
            ['Number', 'Name', 'Project ID', 'Project status'])
        for dictionary in show_projects_dict:
            ...
            t_projects.add_row(
                [projects_count, project_name, project_id, project_status])
        return t_projects
    else:
...
def gns3_show_available_nodes(gns3_server, project_id):
    ...
    r_get_nodes = requests.get(
        gns3_server + '/v2/projects/' + str(project_id) + '/nodes')
    r_get_nodes_dict = r_get_nodes.json()
    count_nodes = 0
    table_available_nodes = PrettyTable(
        ['Number', 'Name', 'Node ID', 'Status', 'Console Port'])
    print()
...
def gns3_show_links(gns3_server, project_id):
    ...
    r_get_links = requests.get(
        gns3_server + '/v2/projects/' + str(project_id) + '/links')
...
def gns3_create_new_project(gns3_server, gns3_code_topology_data):
    ...
    create_project = requests.post(
        gns3_server + '/v2/projects', data=payload_create_project)

    if create_project:
        create_project_dict = create_project.json()
        new_project_id = create_project_dict['project_id']
        print('#' * 100)
        print('A new GNS3 project', '[' + project_name + ']',
              'is created.', 'Project ID:', new_project_id)
        print('#' * 100)
    ...
    for dictionary_node in r_get_nodes_dict:
        if dictionary_node['status'] == 'stopped':
            ...
            if dictionary_node['node_type'] == 'vpcs':
                print(dictionary_node['name'], 'is starting.',
                      'node-id:', dictionary_node['node_id'])
                print('Starting time is 5 sec.')
                node_id = dictionary_node['node_id']
                url_start_node = gns3_server + '/v2/projects/' + \
                    str(project_id) + '/nodes/' + node_id + '/start'
...
def gns3_create_nodes(gns3_server, project_id, gns3_code_topology_data):
    ...
    if not list_node_names:
        print()
        print('All nodes were already created in GNS3 project.',
            ...
            for key, value in gns3_appliances.items():
                if node_image == key:
                    ...
                    if node_image == 'Cloud':
                        ...
                        vpcs_r = requests.post(
                            gns3_server + '/v2/projects/' + project_id + '/nodes', data=vpcs_payload)
                        try:
                        ...
                        nat_r = requests.post(
                            gns3_server + '/v2/projects/' + project_id + '/nodes', data=nat_payload)
                        try:
...
def gns3_create_many_links(gns3_server, project_id, gns3_code_topology_data):
    ...
    for node1, node2, node1_link, node2_link in zip(node1_list_devices,
        ...
        if r_create_link_pair:
            ...
            print(payload_create_link)
            print(r_create_link_pair.text)
            print(
                'A link pair is already created in the GNS3 project. Project ID:', project_id)
            continue
    print('=' * 100)
...
def read_cmd_list(file_path):
    ...
    with open(file_path, "r") as fin:
        cmd_list = [[line.strip(), ".*?"] for line in fin]
    return cmd_list
            ...
            if dictionary_node['name'] == node_name:
                ...
                if dictionary_node['node_type'] == 'vpcs':
                    ...
                    net_connect.send_config_from_file(
                        config_file=config_path, exit_config_mode=False)
                ...
                # For VyOS.
                    ...
                    vyos = pexpect.spawn(
                        'telnet ' + gns3_host_ip + ' ' + str(telnet_port))
                    vyos.expect('')
                    vyos.sendline('\n')
                    vyos.expect('login: ')
                    vyos.sendline('vyos\n')
        ...
        for node2 in list_mgmt_nodes:
            ...
            def func_random_link_mgmt_sw():
                while mgmt_sw_ints_free:
                    ...
                    print()
                    print(random_link_mgmt_sw,
                          'random interface for', mgmt_sw_name)
            ...
            payload = '{"nodes": [{"adapter_number": 0, "node_id": ' + str(mgmt_sw_id) + \
                      ...
                      str(adapter_number_2) + ', "node_id": ' + \
                str(node2_id) + ', "port_number": 0}]}'
...
#######################
    ...
    gns3_send_start_config_telnet(
        gns3_server, project_id, gns3_code_topology_data, global_delay_factor)
